...
<body>
    ...
    if (isset($_POST['kode_mk'])) {
        ...
        $_kode_mk= $_GET['kode_mk'];
        $strSQL ="SELECT * FROM matakuliah WHERE kode_mk='".$_kode_mk."'";
        $runStrSQL = mysqli_query($conn,$strSQL);
        $jmlRowData = mysqli_num_rows($runStrSQL);
        if ($jmlRowData > 0 ){
            while($row = mysqli_fetch_assoc($runStrSQL)){
                $_nama_mk= $row["nama_mk"];
                $_kategori_mk= $row["kategori_mk"];
                $_SKS = $row["SKS"];
            }
        }
    } 
    else {
        $janganiseng = "disabled";
        $_nama_mk="";
        $_kategori_mk="";
        $_SKS = "";
        $_kode_mk="";
    ...
    <!-- ini end modal -->
        ...
        <form id="myform" method="post" action="registrasi_mk2.php">
            <div class="form-group">
                ...
                <input id="kode_mk" class="form-control" type="text" name="kode_mk" value="<?php echo $_kode_mk ?>" readonly <?php echo $janganiseng ?> >
            </div>
            <div class="form-group">
                <label>Nama Mata Kuliah</label>
                <input id="nama_mk" class="form-control" type="text" name="nama_mk" value="<?php echo $_nama_mk ?>" <?php echo $janganiseng ?>>
            </div>
            <div class="form-group">
                <label>Kategori Mata Kuliah</label>
                <select id="kategori_mk" name="kategori_mk" class="form-control" <?php echo $janganiseng ?>>
            ...
            <div class="form-group">
                <label>SKS</label>
                <input id="SKS" class="form-control" type="text" name="SKS" value="<?php echo $_SKS ?>" <?php echo $janganiseng ?>>
            </div>           
